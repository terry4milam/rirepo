 ...
 * 
 * input lines:
 * Seat 3: poker900500 (9,616.16 in chips)
 * Seat 3: avata_07
 * Seat 4: 007Bond (6,000.00 in chips)
 * Seat 4: avata_06
 * ...
 ...
 * lines input:
 * posts ante 12.00 5
 * Seat 2: posts small blind 15.00 (btn click 0.00) allin:0
 * Seat 3: posts big blind 30.00 (btn click 0.00) allin:0
 * Seat 4: posts post blind 30.00 (btn click 0.00) allin:0
 * NOTE:: posts ante 12.00 5 -> Todos os players (5) postam ante de 12
 * 
 * output:
...
const makePosts = (lines, seatsNames) => {
    ...
    const makeAntes = anteLine => {
        ...
        const ante = /(?<=ante ).+(?= \d)/.exec(anteFormatedLine);
    ...
    const postsLines = lines.filter(v => {
        ...
        const isSmall = /: posts small blind /.test(v);
        const isBig = /: posts big blind /.test(v);

        // NOTE:: Funcionou right away com pretendido, mesmo sem suporte previo
        const isPost = /: posts post blind /.test(v);
    ...
    const posts = postsLines.map(line => {
        ...
        const indexOfStartParentheses = numberFormatedLine.lastIndexOf('(');

        const withoutClickTime = numberFormatedLine.slice(0, indexOfStartParentheses - 1);
...
const makeActivity = (lines, seatsNames) => {
    ...
    const activity = lines.filter(line => {
        ...
        if (/^Seat \d: check /.test(commalessLine)) return true;
        if (/^Seat \d: fold /.test(commalessLine)) return true;
        if (/^Seat \d: \d+\.\d+ raise /.test(commalessLine)) return true;
        if (/^Seat \d: \d+\.\d+ call /.test(commalessLine)) return true;
        if (/^Seat \d: \d+\.\d+ bets /.test(commalessLine)) return true;

        if (/^Seat \d: all-in raise \d+\.\d+ /.test(commalessLine)) return true;
        if (/^Seat \d: all-in \d+\.\d+ /.test(commalessLine)) return true;
    ...
    const patchActivity = activity.map((line, index) => {
        ...
        // NOTE:: O amount "raise to" está em (btn click 114.00), "calls" e "bets" é o 
        // primeiro valor, o (btn click 0.00) está a zero
        const possibleAmount = line.includes('raise')
            ? /(?<=btn click )\d+\.\d+/.exec(commalessLine)
    ...
    return patchActivity.map(line => {
        ...
        return line.replace(/Seat \d/, name);
    });
};
